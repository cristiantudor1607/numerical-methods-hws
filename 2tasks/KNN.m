...
function prediction = KNN (labels, Y, test, k)
	...
	% salveaza noul vector ordonat, si un vector de indecsi cu vechile pozitii
	[new_dist, order] = sort(distance);
	
	% iau doar primele k elemente
	new_dist = new_dist(1 : k);
	order = order(1 : k);

	predictions = labels(order(1 : k));
